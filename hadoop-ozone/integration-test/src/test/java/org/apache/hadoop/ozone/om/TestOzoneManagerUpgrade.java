...
/**
 * Test OM prepare upgrade against actual mini cluster.
 */
public class TestOzoneManagerUpgrade extends TestOzoneManagerHA {

  @Test
  public void testPrepareUpgrade() throws Exception {
    ...
    // Restart all OMs in upgrade mode & prepare them for upgrade.
    cluster.getOzoneManagersList().forEach(cluster::shutdownOzoneManager);

    for (int i = 0; i < cluster.getOzoneManagersList().size(); i++) {
      OzoneManager om = cluster.getOzoneManager(i);
      om.restartInUpgradeMode();
      assertTrue(logFilesPresentInRatisPeer(om));
      assertTrue(om.prepareForUpgrade());
      assertFalse(logFilesPresentInRatisPeer(om));
      om.stop();
    }

    // Restart All OMs in regular mode.
    cluster.restartOzoneManager();

    OzoneManager ozoneManager = cluster.getOzoneManager(index);
    ...
    /**
     * ozone om stop (Old version)
     * ozone om --prepareUpgrade (for all OMs) (Old version)
     * ozone om start (New version)
     */
  }
